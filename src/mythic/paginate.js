...
import { map, divide, gt, lt, slice, multiply, subtract,  length } from 'ramda'
...
/// ceiling :: Interger -> Interger
/// ===============================
/// returns the smallest integer greater
/// than or equal to a given number
let ceiling = Math.ceil


/// pagerCount :: (Stream, Interger) -> Interger
/// ===========================================
/// returns the smallest integer greater
/// than or equal to a given number
let pagerCount = (stream, records) => ceiling(divide(length(stream), records))


/// pagerPosition :: (Stream, Interger) -> Interger
/// ===============================================
/// ...
let pagerPosition = (page, records) => multiply(page, records)


/// pagerStart :: (Stream, Interger) -> Interger
/// ============================================
/// ...
let pagerStart = (position, records) => subtract(position, records)


/// pager :: Stream -> Interger -> Interger -> Array
/// ================================================
/// ...
let pager = (stream) => (records=10) => (initialPage=1) => {
    let pageCount = pagerCount(stream, records)

    // http://ramdajs.com/docs/#cond
    if (lt(initialPage, 1)) var page = 1
    if (gt(initialPage, pageCount)) var page = pageCount

    let position = pagerPosition(page, records)
    let start = pagerStart(position, records)

    let pager = slice(start, position, stream)

    return pager
}


/// pageIt :: Stream -> Interger -> Array
/// =====================================
/// ...
let pageIt = pager(objJson)(5)


window.pageIt = pageIt
